...
import com.afunms.data.service.Tru64DataService;
...
/**
 * 
 * ClassName: Tru64DataManager.java
 * <p>
 * 
 * @author 吴品龙
 * @version v1.01
 ...
 */
public class Tru64DataManager extends NodeDataManager {

	private static final SysLogger sysLogger = SysLogger
			.getLogger(Tru64DataManager.class.getName());
	...
	public String getInterfaceInfo() {
		getBaseInfo();
		Tru64DataService tru64DataService = getTru64DataService();
		...
		Vector interfaceInfo = tru64DataService.getInterfaceInfo(orderFlag,
				sorttype);
        ...
        try {
        	list = portconfigDao.loadByIpaddress(tru64DataService.getNodeDTO().getIpaddress());
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            portconfigDao.close();
		...
		return "/detail/host/tru64/interface.jsp";
	}
	...
	public String getPerformanceInfo() {
		...
		Hashtable<String, String> curDayPingValuHashtable = tru64DataService
		...
		// 开始获取响应时间值
		Hashtable<String, String> curDayResponseTimeValueHashtable = tru64DataService
		...
		// 开始获取响应时间柱状图
		curDayResponseTimeBarImage = tru64DataService
				.getCurDayResponseTimeBarImageInfo(curResponseTimeValue,
						curDayResponseTimeMaxValue, curDayResponseTimeAvgValue);

		// 开始获取CPU的图片
		Hashtable<String, String> curDayCPUValueHashtable = tru64DataService
		...
		curDayCPUMaxImageInfo = tru64DataService
		...
		curDayCPUAvgImageInfo = tru64DataService
				.getCurDayCPUAvgImageInfo(curDayCPUAvgValue);

		// 获取当前设备信息列表
		currDeviceNodeTempList = tru64DataService.getCurDeviceListInfo();

		// 获取当天内存
		Hashtable[] curDayAllMemoryValuHashtables = tru64DataService
		...
		// 获取CPU详细信息
		curCPUPerValueHashtable = tru64DataService
				.getCurCPUPerValueHashtable();
		curCPUDetailValueHashtable = tru64DataService
				.getCurCPUDetailValueHashtable();
		curCPUDetailAmChartInfo = tru64DataService.getCurCPUDetailAmChartInfo(
				curCPUPerValueHashtable, curCPUDetailValueHashtable);

		// 获取换页率详细信息
		pageingUsed = (String) request.getAttribute("pageingUsed");
		curPageDetailHashtable = tru64DataService.getCurPageDetailHashtable();
		...
		curPageValueImageInfo = tru64DataService
				.getCurPageValueImageInfo(pageingUsed);
		curDayPageMaxValueImageInfo = tru64DataService
				.getCurDayPageMaxValueImageInfo(curDayPageMaxValue);
		curDayPageAvgValueImageInfo = tru64DataService
		...
		return "/detail/host/tru64/performance.jsp";
	}

	public String getFileSystemInfo() {
		getBaseInfo();
		Tru64DataService tru64DataService = getTru64DataService();
		Hashtable<Integer, Hashtable<String, String>> curDiskValueHashtableInfo = tru64DataService
				.getCurDiskValueHashtableInfo();
		String curDiskValueImageInfo = tru64DataService
				.getCurDiskAmChartInfo(curDiskValueHashtableInfo);

		List<Hashtable<String, String>> curDsikPerfInfo = tru64DataService
				.getCurDiskPerfInfo();

		List<Hashtable<String, String>> curDsikPerfIoInfo = null;
		// TODO disk IO 没有做
		Hashtable<String, String> curPagePerfInfo = tru64DataService
				.getCurPagePerfInfo();
		...
		return "/detail/host/tru64/filesystem.jsp";
	}

	public String getConfigInfo() {
		getBaseInfo();
		Tru64DataService tru64DataService = getTru64DataService();
		List<Nodecpuconfig> curCPUConfigInfo = tru64DataService
				.getCurCPUConfigInfo();

		List<Hashtable<String, String>> curNetmediaConfigInfo = tru64DataService
				.getCurNetmediaConfigInfo();

		Vector<Usercollectdata> curUserConfigInfo = tru64DataService
		...
		return "/detail/host/tru64/config.jsp";
	}

	public String getArpInfo() {
		getBaseInfo();
		Tru64DataService tru64DataService = getTru64DataService();
		List<IpMac> ipMacList = tru64DataService.getCurAllIpMacInfo();
		request.setAttribute("ipMacList", ipMacList);
		return "/detail/host/tru64/arp.jsp";
	}

	public String getSoftwareInfo() {
		getBaseInfo();
		Tru64DataService tru64DataService = getTru64DataService();
		List<SoftwareNodeTemp> softwareNodeTempList = tru64DataService
				.getSoftwareInfo();
		request.setAttribute("softwareNodeTempList", softwareNodeTempList);
		return "/detail/host/tru64/software.jsp";
	}

	public String getServiceInfo() {
		getBaseInfo();
		Tru64DataService tru64DataService = getTru64DataService();
		List<Hashtable<String, String>> serviceNodeTempList = tru64DataService
				.getServiceInfo();
		request.setAttribute("serviceNodeTempList", serviceNodeTempList);
		return "/detail/host/tru64/service.jsp";
	}

	public String getDeviceInfo() {
		getBaseInfo();
		Tru64DataService tru64DataService = getTru64DataService();
		List<DeviceNodeTemp> deviceNodeTempList = tru64DataService
				.getDeviceInfo();
		request.setAttribute("deviceNodeTempList", deviceNodeTempList);
		return "/detail/host/tru64/device.jsp";
	}

	public String getStorageInfo() {
		getBaseInfo();
		Tru64DataService tru64DataService = getTru64DataService();
		List<StorageNodeTemp> storageNodeTempList = tru64DataService
				.getStorageInfo();
		request.setAttribute("storageNodeTempList", storageNodeTempList);
		return "/detail/host/tru64/storage.jsp";
	}

	public String getRouterInfo() {
		getBaseInfo();
		Tru64DataService tru64DataService = getTru64DataService();
		List<RouterNodeTemp> routerNodeTempList = tru64DataService
				.getCurRouterInfo();
	...
	public String getIpListInfo() {
		getBaseInfo();
		Tru64DataService tru64DataService = getTru64DataService();
		List<IpAlias> ipAliasList = tru64DataService.getCurIpListInfo();
	...
	public String getSyslogInfo() {
		...
		return "/detail/host/tru64/syslog.jsp";
	}
	...
	public String getEventInfo() {
		...
		request.setAttribute("startdate", startdate);
		request.setAttribute("todate", todate);
		return "/detail/host/tru64/event.jsp";
	}
	...
	public void getBaseInfo() {
		...
		String nodeid = getNodeid();
		Tru64DataService tru64DataService = getTru64DataService();
		NodeDTO node = tru64DataService.getNodeDTO();
		HostNode hostNode = (HostNode) tru64DataService.getBaseVo();
		category = String.valueOf(hostNode.getCategory());
		maxAlarmLevel = tru64DataService.getMaxAlarmLevel();
		String supperName = tru64DataService.getSupperName();

		// 获取设备的配置信息
		Nodeconfig nodeconfig = tru64DataService.getNodeConfig();
		mac = nodeconfig.getMac();
		processorNum = nodeconfig.getNumberOfProcessors();
		CSDVersion = nodeconfig.getCSDVersion();

		curPageValueHashtable = tru64DataService.getCurPageValueHashtable();
		...
		List<NodeTemp> systemList = tru64DataService.getSystemInfo();
		...
		// 当前连通率和响应时间
		List<NodeTemp> pingList = tru64DataService.getPingInfo();
		...
		curResponseTimeValue = curResponseTimeValue.replaceAll("毫秒", "")
				.replaceAll("%", "");
		curPingImage = tru64DataService.getPingImageInfo(curPingValue);

		// 当前虚拟内存和交换内存
		List<NodeTemp> curMemoryValueList = tru64DataService.getMemoryInfo();
		if (curMemoryValueList != null && curMemoryValueList.size() > 0) {
			...
			curPhysicalMemoryImage = tru64DataService
					.getMemoryImageInfo(Double.valueOf(curPhysicalMemoryValue));
		}

		// 获取当前CPU利用率
		curCPUValue = tru64DataService.getCPUInfo();
		if (curCPUValue != null && curCPUValue.length() > 0) {
			curCPUValueDouble = Double.valueOf(curCPUValue);
			curCPUImage = tru64DataService.getCPUImageInfo(curCPUValueDouble
					.intValue());
		}

		// 内存值
		curDayMemoryValueHashtable = tru64DataService
				.getCurDayMemoryValueHashtable();
		// 磁盘值
		curDayDiskValueHashtable = tru64DataService
				.getCurDayDiskValueHashtable();
		// 内存图
		curDayAmMemoryChartInfo = tru64DataService
				.getAmMemoryChartInfo(curDayMemoryValueHashtable);
		// 磁盘图
		curDayAmDiskChartInfo = tru64DataService
	...
	private String getProcessInfo() {
		...
		if (order.equalsIgnoreCase("CpuTime")) {
			...
			((HostLastCollectDataManager) hostlastmanager).setCpuTime(false);
		}
		try {
		...
		return "/detail/host/tru64/process.jsp";
	}
	...
	private Tru64DataService getTru64DataService() {
		String nodeid = getNodeid();
		String type = getType();
		String subtype = getSubtype();
		return new Tru64DataService(nodeid, type, subtype);
	}
        ...
        if (type.equals("CpuTime")) {
            Collections.sort(list, new Comparator() {
                public int compare(Object o1, Object o2) {
                    ...
                    float cpuTime1 = 0;
                    float cpuTime2 = 0;
                    if (p1.getCpuTime() != null) {
                    	cpuTime1 = (Float) p1.getCpuTime();
                    }
                    if (p2.getCpuTime() != null) {
                    	cpuTime2 = (Float) p2.getCpuTime();
                    }
            ...
            Collections.sort(list, new Comparator() {
                ...
                public int compare(Object o1, Object o2) {
                    ...
                    if ((Float) p1.getCpuUtilization() < (Float) p2
                            .getCpuUtilization()) {
                    	result = 1;
                    }else if((Float) p1.getCpuUtilization() == (Float) p2
                            .getCpuUtilization()){
                    	result = 0;
                    } else {
                    	result = -1;
            ...
            Collections.sort(list, new Comparator() {
                ...
                public int compare(Object o1, Object o2) {
					...
					if ((Float) p1.getMemory() < (Float) p2.getMemory()){ 
						result = 1;
					}else if((Float) p1.getMemory() == (Float) p2.getMemory()){
						result = 0;
					}else{
						result = -1;
					}
            ...
            Collections.sort(list, new Comparator() {
                ...
                public int compare(Object o1, Object o2) {
                    ...
                    if ((Float) p1.getMemoryUtilization() < (Float) p2
                    	...
                    	result = 1;
                    } else if((Float) p1.getMemoryUtilization() == (Float) p2
                            .getMemoryUtilization()){
                    	result = 0;
                    }else {
                    	result = -1;
