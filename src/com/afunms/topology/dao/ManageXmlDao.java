...
public class ManageXmlDao extends BaseDao implements DaoInterface
{
   ...
   public List loadByPerAll(String bid)
   {
    ...
    public BaseVo findById(int id)
    {
	    ...
	    * 按ID找一条记录，ID可以为任意
	    */
	 public BaseVo findById1(int id){
		 BaseVo vo = null;
	     try
		    {
			    rs = conn.executeQuery("select * from topo_manage_xml where id=" + id); 
			    if(rs.next())
			        vo = loadFromRS(rs);
		    }    
		    catch(Exception ex)
...
//	 根据id查找视图名称-----quzhi
	...
	public List findByTopoTypeAndBid(int topotype, String[] bids){
        List list = new ArrayList();
        try {
            String sql = "select * from topo_manage_xml where topo_type=" + topotype;
	...
	public ManageXml findByHomeView() {
		ManageXml xmlVo = new ManageXml();
		try {
			 String sql = "select * from topo_manage_xml where home_view='1'";
			 rs = conn.executeQuery(sql); 
			 if(rs.next()) {
				 xmlVo = (ManageXml) loadFromRS(rs);
			 }
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
