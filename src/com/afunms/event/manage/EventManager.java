...
//	}
	 ...
	 * nielin modify 2010-10-11
	 * 
	 * 将该方法修改为：根据页面的选择 一次性对所有的该类型的告警信息进行处理完毕
	 * 
	 * 如需还原，替换为上面注释的同名方法即可
	 * 
	 * 处理事件
	...
	private String accfi() {
		...
		/**
		 * 	根据当前选中的告警信息  为了查询其同类信息 获取当前告警的全部信息
		 */
		String eventid = getParaValue("eventid");
	...
	/**
	 * 	根据eventid获取该告警信息同类的告警信息
	 * 
	 * 	查询同类告警信息的条件：设备id号、指标类型、告警等级
	 * 	@param eventid
	...
	private List<EventList> getEventsByEventId(String eventid){
		...
		try{
			...
			SysLogger.error("in EventManager.getEventsByEventId()中获取同类告警信息出错", e);
			events = null;
		} finally {
			dao.close();
		}
	...
	private boolean changeManagerSign(List<EventList> events, String managesignTo){
		...
		try{
			...
			if(managersignFrom >= 0){
				result = dao.batchUpdataManagesignByIds(managersignFrom+"", managesignTo, ids);	// 批量更新状态
			}
		} catch (Exception e){
			e.printStackTrace();
			result = false;
	 ...
	 * 填写报告
	 * 
	 *	将该方法修改为：根据页面的选择 一次性对所有的该类型的告警信息的报告处理完毕
	 *
	 *	根据eventid或获取其同类告警信息，依次根据每个告警信息创建对应的填写报告
	 * 
	 *  如需还原，替换为下面注释的同名方法即可
	 * 
	 * @return
