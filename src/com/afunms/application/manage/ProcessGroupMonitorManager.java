...
public class ProcessGroupMonitorManager extends BaseManager implements ManagerInterface{
	...
	public String execute(String action) {
		...
		if("list".equals(action)){
			...
			return showlist();
		}else if ("performlist".equals(action)){
			return performlist();
	...
	public String performlist(){
		String jsp = "/application/processgroup/performlist.jsp";
		try {
			setTarget(jsp);
			ProcessGroupDao processGroupDao = new ProcessGroupDao();
...
//		request.setAttribute("nodeid", nodeid);
	...
	public String chooseNode(){
		...
		try {
			...
			List<BaseVo> list = nodeUtil.getNodeByTyeAndSubtype(Constant.TYPE_HOST, "");	//wupinlong modify 添加主机进程改为所有的Host
