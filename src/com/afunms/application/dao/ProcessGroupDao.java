...
public class ProcessGroupDao extends BaseDao implements DaoInterface {
	...
	public boolean save(BaseVo vo) {
		...
		sql
				.append("insert into nms_process_group(id,ipaddress,name,nodeid,mon_flag,alarm_level)values('");
	...
	public Hashtable<String, String> findByImport(String nodeId) {
//		ArrayList<Hashtable<String, String>> list = new ArrayList<Hashtable<String, String>>();
		Hashtable<String, String> proNameHash = new Hashtable<String, String>();
		try {
			String sql = "select b.name from nms_process_group a left join nms_process_group_config b on a.id=b.group_id where mon_flag='1' and nodeid='"
					+ nodeId + "'";
			rs = conn.executeQuery(sql);
			while (rs.next()) {
				String proName = rs.getString("name");
				proNameHash.put(proName.trim(), proName.trim());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
