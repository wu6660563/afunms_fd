    ...
    private String ready_multi_modify() {
        ...
        if (ids != null) {
            ...
            } else// 否则进入批量修改
            {
                ...
                return "/config/vpntelnet/multi_telnetmodifypassword.jsp";
                /*
                 * for(int i = 1;i <list.size();i++) { Huaweitelnetconf t1 = (Huaweitelnetconf)list.get(i); //判断所选设备的初始密码是否相同，只有在相同的情况下才能执行批量操作
                 ...
                 * break; } } if(num == list.size())//相等 说明满足条件，可以进行批量处理 { request.setAttribute("vpntelnetconf", vo);
                 * request.setAttribute("ipAddresses", s2); if(vo.getDeviceRender().equals("h3c")) { request.setAttribute("deviceType", "h3c"); return
                 * "/config/vpntelnet/multi_telnetmodifypassword.jsp"; } else//if(vo.getDeviceRender().equals("cisco"))//无法测试，未操作 { return ""; } }
...
//        }
    ...
    private String modifyTelnetPasswordForBatch() {
        ...
        /*
         * while(threadPool.getActiveCount() == 0)//返回主动执行任务的近似线程数,为0 说明所有任务已经执行完 { try{ Thread.sleep(1000); }catch(Exception e){e.printStackTrace();}
         * }
         */
        System.out.println(result.toString());
        request.setAttribute("result", result.toString());
    ...
    private String synchronizeData() {
        HaweitelnetconfDao hdao = new HaweitelnetconfDao();
        // 把表topo_node_telnetconfig 中 is_synchronized=0 并且 ip_address=topo_host_node.ip_address 的数据项的id设置为表topo_host_node 中的 id
...
//    }
     ...
     * tvpn.setSuuser(vo.getSuuser());// su tvpn.setSupassword(vo.getSupassword());// su密码 tvpn.setUser(vo.getUser());// 用户
     * tvpn.setPassword(vo.getPassword());// 密码 tvpn.setIp(vo.getIpaddress());// ip地址 tvpn.setDEFAULT_PROMPT(vo.getDefaultpromtp());// 结束标记符号
