...
public class DiskconfigDao extends BaseDao implements DaoInterface
{
    ...
    public Diskconfig loadDiskconfig(int id)
    {
  	...
  	public Diskconfig getByipandName(String ip,String name)
  	{
  		...
  		try
  		{
  			rs = conn.executeQuery("select * from nms_diskconfig where ipaddress='"+ip+"' and name ='"+name+"' and bak='利用率阈值'");
  			if(rs.next())
  				return (Diskconfig)loadFromRS(rs); 
  		}
  		catch(Exception e)
...
//				}
				if (!diskhash.containsKey(diskdata.getIpaddress()+":"+diskdata.getSubentity())){
					...
					*/
					...
					diskconfig.setSms1(new Integer(1));// 0：不告警 1：告警，默认的情况是不发送短信
					diskconfig.setSms2(new Integer(1));// 0：不告警 1：告警，默认的情况是不发送短信
					diskconfig.setSms3(new Integer(1));// 0：不告警 1：告警，默认的情况是不发送短信
					try{
