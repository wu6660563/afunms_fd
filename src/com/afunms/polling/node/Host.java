...
//	            				}
	                			if(flag>0){
	                				...
	                				if(status < flag)status = flag;
//	                				try{
//	                					createSMS("network","ping",ipAddress,getId()+"",ipAddress+" "+nm.getAlarm_info()+"当前值:"+pingvalue+" 阀值:"+nm.getLimenvalue0(),flag,0,1);
//	                				}catch(Exception e){
//	                					e.printStackTrace();
//	                				}
	                				//createSMS("host","ping",);
	                				//生成事件
	                				//createEvent("poll",sysLocation,getBid(),ipAddress+" "+nm.getAlarmInfo()+"当前值:"+cpuvalue+" 阀值:"+nm.getLimenvalue0(),flag,"host","ping",ipAddress,getId()+"");
	                				if(this.alarmlevel < flag)this.alarmlevel=flag;
	                			}else{
	                				...
	                				if (createeventdata.containsKey("network:ping:"+ipAddress)){
	                					...
	                					try{
	                						createSMS("network","ping",ipAddress,getId()+"","故障恢复:"+ipAddress+" "+nm.getAlarm_info()+"的故障已恢复",0,0,0);
	                					}catch(Exception e){
	                    					e.printStackTrace();
	                    				}
	                				}
...
//	        				}
	        				if(flag>0){
	            				//有告警产生
	            				alarmMsg.append(nm.getAlarm_info()+"当前值:"+inhdx+" 阀值:"+nm.getLimenvalue0()+"<br>");
	            				alarm = true;
	            				if(status < flag)status = flag;
	            				if(this.alarmlevel < flag)this.alarmlevel=flag;
	            			}
		    							...
		    							if(interfacedata.getCategory().equalsIgnoreCase("Interface")&& interfacedata.getEntity().equalsIgnoreCase("ifOperStatus") && interfacedata.getSubentity() != null){
		    								...
		    								if(portconfigHash.containsKey(interfacedata.getSubentity())){
		    									...
		    									if (!"up".equalsIgnoreCase(interfacedata.getThevalue())){
		    			            				...
		    			            				//SysLogger.info("端口 "+portconfig.getName()+" "+portconfig.getLinkuse()+" down"+"<br>");
		    			            				alarm = true;
...
//	        	            				}
	        		        				if(flag>0){
	        		            				//有告警产生
	        		            				alarmMsg.append("第"+inpacks.getSubentity()+"端口"+nm.getAlarm_info()+nm.getLimenvalue0()+"<br>");
	        		            				alarm = true;
	        		            				if(status < flag)status = flag;
	        		            				//生成事件
//	        		            				try{
//	        		            					createSMS("network","interface",ipAddress,getId()+"",ipAddress+" "+"第"+inpacks.getSubentity()+"端口"+nm.getAlarm_info()+"当前值:"+inpacks.getThevalue()+" 阀值:"+nm.getLimenvalue0(),flag,0,1);
//	        		            				}catch(Exception e){
//	        		            					e.printStackTrace();
//	        		            				}
	        		            				//createEvent("poll",sysLocation,getBid(),"第"+inpacks.getSubentity()+"端口"+nm.getAlarmInfo()+"当前值:"+inpacks.getThevalue()+" 阀值:"+nm.getLimenvalue0(),flag,"network","interface");
	        		            				if(this.alarmlevel < flag)this.alarmlevel=flag;
	        		            			}
	        							}
...
//	        	            				}
	        		        				if(flag>0){
	        		            				//有告警产生
	        		            				alarmMsg.append("第"+outpacks.getSubentity()+"端口"+nm.getAlarm_info()+nm.getLimenvalue0()+"<br>");
	        		            				alarm = true;
	        		            				if(status < flag)status = flag;
	        		            				//生成事件
//	        		            				try{
//	        		            					createSMS("network","interface",ipAddress,getId()+"",ipAddress+" "+"第"+outpacks.getSubentity()+"端口"+nm.getAlarm_info()+"当前值:"+outpacks.getThevalue()+" 阀值:"+nm.getLimenvalue0(),flag,0,1);
//	        		            				}catch(Exception e){
//	        		            					e.printStackTrace();
//	        		            				}
	        		            				//createEvent("poll",sysLocation,getBid(),"第"+outpacks.getSubentity()+"端口"+nm.getAlarmInfo()+"当前值:"+outpacks.getThevalue()+" 阀值:"+nm.getLimenvalue0(),flag,"network","interface");
	        		            				if(this.alarmlevel < flag)this.alarmlevel=flag;
	        		            			}
	        							}
...
//	            				//生成事件
//	            				try{
//	            					createSMS("host","cpu",ipAddress,getId()+"",ipAddress+" "+nm.getAlarmInfo()+"当前值:"+cpuvalue+" 阀值:"+nm.getLimenvalue0(),flag,0,1);
//	            				}catch(Exception e){
//	            					e.printStackTrace();
//	            				}
	            				//createEvent("poll",sysLocation,getBid(),nm.getAlarmInfo()+"当前值:"+cpuvalue+" 阀值:"+nm.getLimenvalue0(),flag,"host","cpu");
	            				if(this.alarmlevel < flag)this.alarmlevel=flag;
	            			}       			
	            			msg.append(nm.getDescr() + ":" + cpuvalue + " "+nm.getThreshlod_unit() + "<br>");
	        			}
	        				...
	        				if (diskVector != null && diskVector.size()>0){
	            				...
	            				for(int si=0;si<diskVector.size();si++){
	            					...
	            					if(diskdata.getEntity().equalsIgnoreCase("Utilization")){
	            						//利用率
	            						//int flag = checkeventutil.checkEvent("disk",new Double(diskdata.getThevalue()),nm,diskdata.getSubentity(),"利用率阈值");
	            						int flag = 0;
...
//	        	            				}
	            	        			...
	            	        			if(flag>0){
	            	        				...
	            	        				if(getSysOid().startsWith("1.3.6.1.4.1.311.1.1.3")){
	            	        					//有告警产生
	                	        				alarmMsg.append(nm.getAlarm_info()+" "+diskdata.getSubentity().substring(0, 3)+" 阀值:"+limevalue+"<br>");
	                	        				alarm = true;
	                	        				if(status < flag)status = flag;
	            	        				}else{
	            	        					//有告警产生
...
//	                    				}
	            	        			if(flag>0){
	            	        				...
	            	        				//生成事件
//	            	        				try{
//	            	        					createSMS("host","memory",ipAddress,getId()+"",ipAddress+" "+nm.getAlarmInfo()+"当前值:"+CEIString.round(new Double(memorydata.getThevalue()).doubleValue(),2)+" 阀值:"+nm.getLimenvalue0(),flag,0,1);
//	            	        				}catch(Exception e){
//	                        					e.printStackTrace();
//	                        				}
