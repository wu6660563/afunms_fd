...
public class Download extends SnmpMonitor implements IndicatorGather {
        ...
        try {
            ...
            if (errorCode <= 0) {
                ...
                for (AlarmIndicatorsNode alarmIndicatorsNode : list) {
                    if ("download".equals(alarmIndicatorsNode.getName())) {
    ...
    public Result getValue(Node node, NodeGatherIndicators nodeGatherIndicators) {
        ...
        int errorCode = FTPUtil.download(ip, port, userName, password, fileName, localPath, remotePath);
