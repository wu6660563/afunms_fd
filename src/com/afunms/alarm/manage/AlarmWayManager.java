...
public class AlarmWayManager extends BaseManager implements ManagerInterface{
	...
	public AlarmWay createAlarmWay(){
		...
		String[] isFvsdAlarm_arr = getParaArrayValue("isFvsdAlarm");
		...
		String isFvsdAlarm = "0";
        if(isFvsdAlarm_arr!=null && isFvsdAlarm_arr.length > 0 && "1".equals(isFvsdAlarm_arr[0])){
            isFvsdAlarm = "1";
        }
		...
		alarmWay.setIsFvsdAlarm(isFvsdAlarm);
		return alarmWay;
	}
	...
	public List createAlarmWayDetailList(){
		...
		for (int i = 1; i <= num; i++) {
			...
			String sendIntervalTimes = request.getParameter("sendIntervalTimes-" + partName);
			...
			if (alarmCategory == null || "".equalsIgnoreCase(alarmCategory)) {
			    continue;
			}
			...
			if(sendIntervalTimes == null || "".equals(sendIntervalTimes)) {
				sendIntervalTimes = "0";
			}
			alarmWayDetail.setSendIntervalTimes(sendIntervalTimes);
