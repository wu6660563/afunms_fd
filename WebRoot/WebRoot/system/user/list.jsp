...
<%
  ...
  User user = (User)session.getAttribute(SessionConstant.CURRENT_USER);
    RoleOperationPermissionService roleOperationPermissionService = RoleOperationPermissionService.getInstance(user, OperationPermissionService.getUserOperationPermission());
...
<table border="0" id="table1" cellpadding="0" cellspacing="0" width=100% height="100%">
	<tr>
			...
			<tr>
				<td background="<%=rootPath%>/common/images/right_t_02.jpg" width="100%"><table width="100%" cellspacing="0" cellpadding="0">
                  <tr>
                    ...
                    <td class="layout_title">系统管理 >> 用户管理 >> 用户列表</td>
		...
		<tr >
			<td colspan="2">
				<table cellspacing="1" cellpadding="0" width="100%">
					...
					<%
    				String sex = "";
    				boolean isOperatorSuperAdmin = false;
    				...
    				for(int i=0;i<list.size();i++) {
       					vo = (User)list.get(i);
       					if (!isOperatorSuperAdmin && vo.getRole() == 0) {
       						continue;
       					}
					...
					<td ><a style="display: <%=updateOperationDisable%>;" href="<%=rootPath%>/user.do?action=ready_edit&id=<%=vo.getId()%>"><img src="<%=rootPath%>/resource/image/editicon.gif" border="0"/></a></td>
  				</tr>
  				<%
  				}
			%>
