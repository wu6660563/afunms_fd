			...
			<tr>
				...
				<td class="td-container-main">
					<table id="container-main" class="container-main">
						<tr>
							<td class="td-container-main-add">
													...
													<td>
														<table id="add-content-header" class="add-content-header">
										                	<tr>
											                	...
											                	<td class="add-content-title">资源&gt;&gt; 设备维护 &gt;&gt; 链路添加</td>
								...
								<TD nowrap align="right" height="24" width="10%">起点设备&nbsp;</TD>				
								<TD nowrap width="40%">&nbsp;
					            <select size=1 name='start_id' style='width:250px;' onchange='doChange()'>
					<%
					  for(int i=0;i<list.size();i++)
					  {
					     HostNode node = (HostNode)list.get(i);
					     String selected = "";
					     if(node.getId()==startId) selected = "selected";
					%>            
					            <option value='<%=node.getId()%>' <%=selected%>><%=node.getAlias()%>(<%=node.getIpAddress()%>)</option>
					<%}%>            
					            </select>			
					            </TD>						
								<TD nowrap align="right" height="24" width="10%">起点端口索引&nbsp;</TD>				
								<TD nowrap width="40%">&nbsp;
					            <select size=1 name='start_index' style='width:250px;' onchange='doChange()'>
					<%
					  while(it1.hasNext())
					  {
					...
					<%}%></select>			
								...
								<tr>						
								<TD nowrap align="right" height="24" width="10%">终点设备&nbsp;</TD>				
								<TD nowrap width="40%">&nbsp;
					            <select size=1 name='end_id' style='width:250px;' onchange='doChange()'>
					<%
					  for(int i=0;i<list.size();i++)
					  {
					     HostNode node = (HostNode)list.get(i);
					     String selected = "";
					     if(node.getId()==endId) selected = "selected";     
					%>            
					            <option value='<%=node.getId()%>' <%=selected%>><%=node.getAlias()%>(<%=node.getIpAddress()%>)</option>
					<%}%>            
					            </select></TD>						
								<TD nowrap align="right" height="24" width="10%">终点端口索引&nbsp;</TD>				
								<TD nowrap width="40%">&nbsp;
					            <select size=1 name='end_index' style='width:250px;' onchange='doChange()'>
					<%
					  while(it2.hasNext())
					  {
