...
<%@page import="com.afunms.polling.om.ProcessInfo"%>
...
<%
    ...
    //wupinlong add 关键进程Hash
    Hashtable<String, String> procNameHash = (Hashtable<String, String>) request.getAttribute("procNameHash");
	...
	String[] processItemch={"进程名称","进程个数","进程类型","cpu时间","内存占用率","内存占用量","当前状态"};	//wupinlong delete "CPU占用率",
	String[] second_processItem={"Pid","User","Name","Type","CpuTime","Memory","MemoryUtilization","Status"};//wupinlong delete "CpuUtilization",
	String[] second_processItemch={"进程ID","用户名","进程名称","进程类型","cpu时间","内存占用量","内存占用率","当前状态"};//wupinlong delete "CPU占用率",
%> 
<html>
<head>
...
{
    mainForm.action="<%=rootPath%>/tru64Data.do?action=getProcessInfo&processName="+pro_name+"&layer="+layer;
    mainForm.submit();
}
function order(para,layer){
    ...
    %>
    mainForm.action = "<%=rootPath%>/tru64Data.do?action=getProcessInfo&processName="+"<%=processName%>" + "&layer="+layer;
    
    <%
        } else {
    %>
     mainForm.action = "<%=rootPath%>/tru64Data.do?action=getProcessInfo&layer="+layer;
    <%
        }
    %>
    mainForm.submit();
...
function setClass(){
    document.getElementById('tru64DetailTitle-4').className='detail-data-title';
    document.getElementById('tru64DetailTitle-4').onmouseover="this.className='detail-data-title'";
    document.getElementById('tru64DetailTitle-4').onmouseout="this.className='detail-data-title'";
}
...
<!-- 右键菜单结束-->
    <form id="mainForm" method="post" name="mainForm">
		...
		<%
            	...
            	{
         %>
																				<td align="center" class="body-data-title"><%=title%></td>
		 <%
		 		}
             }
         }
         ...
         %>
            ...
            %>
            		<tr height="28" bgcolor="#FFFFFF"
																				...
																				<td class="detail-data-body-list" align="center"><%=processInfo.getStatus() %></td>
																				<td class="detail-data-body-list" align="center">
																					<%if(procNameHash.containsKey(processInfo.getName())) {
																					%>
																					<input type="button" Class="button"
