...
<%
	...
	String[] second_processItem={"Pid","User","Name","Type","CpuTime","CpuUtilization","Memory","MemoryUtilization","Status"};//wupinlong delete "CpuUtilization",
...
{
    mainForm.action="<%=rootPath%>/hpunixData.do?action=getProcessInfo&processName="+pro_name+"&layer="+layer;
    mainForm.submit();
}
function order(para,layer){
    ...
    %>
    mainForm.action = "<%=rootPath%>/hpunixData.do?action=getProcessInfo&processName="+"<%=processName%>" + "&layer="+layer;
    
    <%
        } else {
    %>
     mainForm.action = "<%=rootPath%>/hpunixData.do?action=getProcessInfo&layer="+layer;
    <%
        }
    %>
    mainForm.submit();
