	...
	if (checkList && checkList.length > 0) {
		...
		if (result) {
			...
			if(reportName == 'nodeAlarmCnt'){
				window.open('<%=rootPath%>/ReportEmitter?rpt=' + reportName + '.brt&params=starttime=' + starttime + ';endtime=' + endtime +';nodeId='+ nodeId +';nodeIp=' + nodeIp + ';nodeName=' + nodeName + ';nodeType='+ nodeType +';nodeSubtype='+ nodeSubtype +';query=' + sqlCondition ,'设备告警报表','height=500,width=800,toolbar=no,menubar=no,location=no,status=no,scrollbars=yes,resizable=yes');
			}else if(reportName == 'nodeAlarmPerHour'){
				window.open('<%=rootPath%>/ReportEmitter?rpt=' + reportName + '.brt&params=starttime=' 
					+ starttime + ';endtime=' + endtime +';query=' + sqlCondition+';nodeNum='+nodeNum ,'设备告警报表','height=500,width=800,toolbar=no,menubar=no,location=no,status=no,scrollbars=yes,resizable=yes');		
			}else if(reportName == 'nodeAlarmPerDay'){
				window.open('<%=rootPath%>/ReportEmitter?rpt=' + reportName + '.brt&params=starttime=' 
					+ starttime + ';endtime=' + endtime +';query=' + sqlCondition ,'设备告警报表','height=500,width=800,toolbar=no,menubar=no,location=no,status=no,scrollbars=yes,resizable=yes');		
			}
		} else {
			alert("请选择设备!!!");
		}
																		...
																		<table id="report-data-body" class="report-data-body">
																			<tr height=50>
																				<td class="report-data-body-title">
																					<input type="button" name="nodeAlarmCnt" value="设备CPU性能报表"
																									onclick="popupReport('nodeAlarmCnt')">
																				<td class="report-data-body-title">
																					<input type="button" name="nodeAlarmPerDay" value="设备内存性能报表"
																									onclick="popupReport('nodeAlarmPerDay')">
																				</td>
																				</td>
																				<td class="report-data-body-title">
																					<input type="button" name="nodeAlarmPerHour" value="设备端口性能报表"
																									onclick="popupReport('nodeAlarmPerHour')">
																				</td>
																				
																				<td class="report-data-body-title">
																					<input type="button" name="nodeAlarmPerHour" value="设备连通率报表"
																									onclick="popupReport('nodeAlarmPerHour')">
																				</td>
																				<td class="report-data-body-title">
																				</td>
