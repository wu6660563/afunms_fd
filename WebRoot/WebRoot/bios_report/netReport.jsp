	...
	if (checkList && checkList.length > 0) {
		for(var i=0; i<checkList.length; i++){
			...
			if(checkbox.checked) {
			    ...
			    }else if(reportName == 'memoryReport'){
			    ...
			    }else if(reportName == 'utilhdx'){
    				var url = "<%=rootPath%>/performanceReport.do?action=port&tableName="+nodeIp[nodeNum]+"&nodeId="+nodeId[nodeNum]+"&nodeType="+nodeType[nodeNum]+"&nodeSubtype="+nodeSubtype[nodeNum]+"&starttime="+starttime+"&endtime="+endtime;
    				msgWindow=window.open(url,"protypeWindow","toolbar=no,width=800,height=600,directories=no,status=no,scrollbars=yes,menubar=no,resizable=yes")
			    }
		...
		if (result) {
			...
			if(reportName == 'cpuReport' || reportName == 'memoryReport' ){
				window.open('<%=rootPath%>/bios_report/reportWrapper.jsp?rpt=netReport.brt&starttime=' + starttime + ';endtime=' + endtime +';tableName='+ tableName ,'设备CPU性能报表','height=650,width=800,toolbar=no,menubar=no,location=no,status=no,scrollbars=yes,resizable=no');
			}
			...
			if(reportName == 'pingReport'){
				...
				window.open('<%=rootPath%>/bios_report/reportWrapper.jsp?rpt=' + reportName + '.brt&starttime=' 
					+ starttime + ';endtime=' + endtime +';query=' + sqlCondition ,'设备告警报表','height=640,width=800,toolbar=no,menubar=no,location=no,status=no,scrollbars=yes,resizable=no');		
			}
		} else {
			alert("请选择设备!!!");
		}
																		...
																		<table id="report-data-body" class="report-data-body">
																			<tr height=50>
																				<td class="report-data-body-title">
																					<input type="button" name="cpuReport" value="设备CPU性能报表"
																									onclick="popupReport('cpuReport')">
																				<td class="report-data-body-title">
																					<input type="button" name="memoryReport" value="设备内存性能报表"
																									onclick="popupReport('memoryReport')">
																				</td>
																				...
																				<td class="report-data-body-title">
																					<input type="button" name="utilhdx" value="设备端口报表"
																									onclick="popupReport('utilhdx')">
																				</td>
																			</tr>		
																			<tr height=28>
																				<td class="report-data-body-title">
