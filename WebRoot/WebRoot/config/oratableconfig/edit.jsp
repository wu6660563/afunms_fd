...
OracleTableSpaceConfig port = (OracleTableSpaceConfig)request.getAttribute("vo");
...
function showdiv(id)
{
  ...
  function toEdit()
  {
     ...
     //{
     	if (confirm("确定要修改吗?")){
        	mainForm.action = "<%=rootPath%>/oratablespaconfig.do?action=update";
        	mainForm.submit();
...
<title>磁盘告警修改 -> <%=port.getIpaddress()%></title>
...
<input type=hidden id="id" name="id" value="<%=port.getId()%>">
			...
			<tr>
			  	<td height=380 bgcolor="#FFFFFF" valign="top" align=center>	
					...
					<table width="80%" style="BORDER-COLLAPSE: collapse" borderColor=#cedefa cellPadding=0 rules=none align=center border=1 algin="center">
                  				<tr>
                    					<td width="619" height="25" bgcolor="#ECECEC" style="HEIGHT: 25px" colspan=2><div align="center"><b>资源管理 >> 性能监视 >>修改数据库表空间监视配置 >> <%=port.getFileName()%></b></div></td>
                  				</tr>
                  				<tr>
                  				...
                  				<tr bgcolor="#F1F1F1">
                  					<TD nowrap align="right" height="24" width="10%">数据库名称&nbsp;</TD>
                  					<TD nowrap width="40%">&nbsp;<%=port.getFileName()%>
                  					</TD>
                				</tr>
                				<tr >
                  					<TD nowrap align="right" height="24" width="10%">告警&nbsp;</TD>
                  					<TD nowrap width="40%">&nbsp;
                  					<select name=monflag>
