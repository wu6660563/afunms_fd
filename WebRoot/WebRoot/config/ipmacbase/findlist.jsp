...
%>
   										<tr bgcolor="#FFFFFF" <%=onmouseoverstyle%> height=25>  
    											...
    											<%
    												...
    												String sms = "<a href="+rootPath+"/ipmacbase.do?action=updateselect&id="+vo.getId()+"&ifsms=0&key="+key+"&value="+value+">取消发送</a>";
    												if(vo.getIfsms().equals("0")){
    													sms="<a href="+rootPath+"/ipmacbase.do?action=updateselect&id="+vo.getId()+"&ifsms=1&key="+key+"&value="+value+">发送</a>";
    												}
    												String tel="<a href="+rootPath+"/ipmacbase.do?action=updateselect&id="+vo.getId()+"&iftel=0&key="+key+"&value="+value+">是</a>";
    												if(vo.getIftel().equals("0")){
    													tel="<a href="+rootPath+"/ipmacbase.do?action=updateselect&id="+vo.getId()+"&iftel=1&key="+key+"&value="+value+">否</a>";
    												}
    												String email="<a href="+rootPath+"/ipmacbase.do?action=updateselect&id="+vo.getId()+"&ifemail=0&key="+key+"&value="+value+">取消发送</a>";
    												if(vo.getIfemail().equals("0")){
    													email="<a href="+rootPath+"/ipmacbase.do?action=updateselect&id="+vo.getId()+"&ifemail=1&key="+key+"&value="+value+">发送</a>";
    												}
    											%>
    											<td height=28><a href="<%=rootPath%>/ipmacbase.do?action=selcancelipmacbase&id=<%=vo.getId()%>&flag=-1">取消基线</a></td>
    											<% if(ifband ==0){%>
            												
            												<td height=28><a href="<%=rootPath%>/ipmacbase.do?action=selsetipmacbase&id=<%=vo.getId()%>&flag=1&key=<%=key%>&value=<%=value%>">绑定</a></td>
            												<td height=28><a href="<%=rootPath%>/ipmacbase.do?action=selsetipmacbase&id=<%=vo.getId()%>&flag=2&key=<%=key%>&value=<%=value%>">绑定</a></td>
            												<td height=28><a href="<%=rootPath%>/ipmacbase.do?action=selsetipmacbase&id=<%=vo.getId()%>&flag=3&key=<%=key%>&value=<%=value%>">绑定</a></td>
            												<%
            												    	}else if(ifband == 1){
            												%>  
            												
            												<td height=28><a href="<%=rootPath%>/ipmacbase.do?action=selcancelipmacbase&id=<%=vo.getId()%>&flag=0&key=<%=key%>&value=<%=value%>">取消</a></td>
            												<td height=28><a href="<%=rootPath%>/ipmacbase.do?action=selsetipmacbase&id=<%=vo.getId()%>&flag=2&key=<%=key%>&value=<%=value%>">绑定</a></td>
            												<td height=28><a href="<%=rootPath%>/ipmacbase.do?action=selsetipmacbase&id=<%=vo.getId()%>&flag=3&key=<%=key%>&value=<%=value%>">绑定</a></td>
            												<%
            												    	}else if(ifband == 2){
            												...
            												<td height=28><a href="<%=rootPath%>/ipmacbase.do?action=selsetipmacbase&id=<%=vo.getId()%>&flag=1&key=<%=key%>&value=<%=value%>">绑定</a></td>
            												<td height=28><a href="<%=rootPath%>/ipmacbase.do?action=selsetipmacbase&id=<%=vo.getId()%>&flag=2&key=<%=key%>&value=<%=value%>">绑定</a></td>
            												<td height=28><a href="<%=rootPath%>/ipmacbase.do?action=selsetipmacbase&id=<%=vo.getId()%>&flag=0&key=<%=key%>&value=<%=value%>">绑定</a></td>
            												<%
            													}
            												%>
