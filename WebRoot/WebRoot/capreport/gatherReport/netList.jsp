	...
	if (checkList && checkList.length > 0) {
		for(var i=0; i<checkList.length; i++) {
			...
			if(checkbox.checked) {
				...
				nodeId +=document.getElementById("nodeid_"+checkboxValue).value + ",";
				nodeType += document.getElementById("type_"+checkboxValue).value + ",";
				nodeSubtype += document.getElementById("subtype_"+checkboxValue).value + ",";
			}
			
		}
		if (result) {
			window.open('<%=rootPath%>/nodeReport.do?action=reportList&starttime=' + starttime + '&endtime=' + endtime +'&nodeId='+ nodeId +'&nodeType='+ nodeType +'&nodeSubtype='+ nodeSubtype ,'设备综合报表','height=500,width=800,toolbar=no,menubar=no,location=no,status=no,scrollbars=yes,resizable=yes');
		} else {
			alert("请选择设备!!!");
		}
	} else {
																	...
																	<td>
																		...
																		<table id="report-data-body" class="report-data-body">
																			<tr height=50>
																				...
																				<td class="report-data-body-title">
																					内存利用率(%):
																					<input type="button" name="memory" id="memory">
																				</td>
																			</tr>
																			<tr height=28>
																				<td class="report-data-body-title">
																			...
																			<tr bgcolor="#ffffff" <%=onmouseoverstyle%> height=25>
																				...
																				<td style="display: none">
																					<input type="hide" id="nodeid_<%=i%>"
																						value=<%=node.getNodeid()%> />
																					<input type="hide" id="ipaddress_<%=i%>"
																						value=<%=node.getIpaddress()%> />
																					<input type="hide" id="name_<%=i%>"
																						value=<%=node.getName()%> />
																					<input type="hide" id="type_<%=i%>"
																						value=<%=node.getType()%> />
																					<input type="hide" id="subtype_<%=i%>"
																						value=<%=node.getSubtype()%> />
																				</td>
																			</tr>
																			<%
																				}
